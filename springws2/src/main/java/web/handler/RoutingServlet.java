...
public class RoutingServlet {
	...
	public RedirectView q1(
		...
		boolean isValid = MathQuestionService.validateNumber(resultUser);
		RedirectView redirectView = null;
		...
		if (isValid && !Double.isNaN(calculatedResult)) {
			...
			if (calculatedResult == Double.valueOf(resultUser)) {
				...
				redirectView = new RedirectView("/q1", true);
				redirectAttributes.addFlashAttribute("message", "Incorrect answer, try again.");
			}		
			return redirectView;
		}
		else {
			redirectView = new RedirectView("/q1", true);
			redirectAttributes.addFlashAttribute("message", "Incorrect Inputs, try again.");
			return redirectView;
		}
	}	
	...
	public String q3View() {		
		...
		public RedirectView q3(HttpServletRequest request, RedirectAttributes redirectAttributes) {
			...
			if (isValid && !Double.isNaN(calculatedResult))  {
				...
				if (calculatedResult == Double.valueOf(resultUser)) {
					redirectView = new RedirectView("/login", true);
					redirectAttributes.addFlashAttribute("message", "STEM Game Completed! Thank you!");
				} else {
					redirectView = new RedirectView("/q3", true);
					redirectAttributes.addFlashAttribute("message", "Incorrect answer, try again.");
				}		
